...
async def google_callback(
    ...
    try:
        # 필수 파라미터 체크
        if not code:
            return RedirectResponse(url=f"{LOGIN_PAGE}?error=ERR_020&message=INVALID_CALLBACK_REQUEST")
        ...
        return RedirectResponse(url=f"{LOGIN_PAGE}?error=ERR_006&message=EMAIL_ALREADY_EXISTS")
    except OnlySnuEmailAllowedException:
        return RedirectResponse(url=f"{LOGIN_PAGE}?error=ERR_010&message=ONLY_SNU_EMAIL_ALLOWED")
    except GoogleAuthFailedException:
        return RedirectResponse(url=f"{LOGIN_PAGE}?error=ERR_019&message=GOOGLE_AUTH_FAILED")
