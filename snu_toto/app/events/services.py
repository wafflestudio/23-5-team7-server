...
class EventServices:
    ...
    async def get_events_paginated(
        ...
        limit: int = 10
    ) -> tuple[List[EventDetailResponse], str | None, bool]:
        """커서 기반 페이지네이션으로 이벤트 목록 조회"""
        import base64
        from snu_toto.app.events.exceptions import InvalidCursorError
        ...
        if cursor:
            try:
                ...
                cursor_end_at, cursor_event_id = decoded.split('|')
                
                # 커서로 지정된 이벤트가 실제로 존재하는지 확인
                cursor_event = await self.event_repositories.get_event_by_id(cursor_event_id)
                if not cursor_event:
        ...
        if has_more and events:
            last_event = events[-1]
            cursor_data = f"{last_event.end_at.isoformat()}|{last_event.event_id}"
            next_cursor = base64.b64encode(cursor_data.encode('utf-8')).decode('utf-8')
        
        return event_details, next_cursor, has_more
